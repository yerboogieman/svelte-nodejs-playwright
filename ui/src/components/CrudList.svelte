<script>
    ...
    async function updateFriend(id) {
        const friendToUpdate = friends.find(friend => friend.id === id); // Find the friend to update by ID
        ...
        const response = await fetch(`${apiUrl}/${id}`, {
            ...
            headers: { 'Content-Type': 'application/json' }, // Set the request headers to indicate JSON content
            body: JSON.stringify(updatedFriend) // Convert the updated friend object to JSON and send it in the request body
        });

        if (response.ok) { // If the response is OK
            await fetchFriends(); // Fetch the updated list of friends
        } else {
            console.error('Failed to update friend'); // Log an error message if the request failed
        }
    }
